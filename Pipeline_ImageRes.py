"""
Combined pipeline for the ImageNet-supervised-pretrained ResNet50 (torchvision,
IMAGENET1K_V2 weights, downloaded via download_imagenet_pretrained_backbones.py
as ./out/imagenet_pretrained/resnet50_supervised_pretrained.pt): linear-probe
accuracy, final RSA, layerwise RSA, and entropy-quartile RSA — all in one script,
mirroring openclip_rn50_rsa_full.py / openclip_vitb16_rsa_full.py in structure.

This is the simplest of the three "combined" scripts: the checkpoint is a plain
torchvision.models.resnet50 state_dict (same class the model was saved from), so
weights load directly with load_state_dict — no OpenCLIP-specific attribute
names, no position-based matching needed, no shape-mismatch risk.

Only the "supervised" paradigm applies here (this backbone has no CLIP/VAE head,
just the 1000-way ImageNet classifier, which is dropped in favor of a fresh
linear probe on the frozen 2048-dim features).

Requires: resnet50_supervised_pretrained.pt already downloaded via
...
ARCH_TAG     = "resnet50_imagenet"   # used in output filenames/folders

CHECKPOINT_PATH = "./out/imagenet_pretrained/resnet50_supervised_pretrained.pt"
...
NUM_CLASSES = 10   # for the CIFAR-10 linear probe (the original 1000-way ImageNet
                    # head is discarded; we probe the frozen 2048-dim features)
EPOCHS      = 15

PARADIGM = "supervised"   # only paradigm this backbone supports

# Layer steps for the layerwise analysis — standard torchvision resnet50 attributes.
LAYER_STEPS = {
    "0pct":   "stem",     # after conv1 + bn1 + relu + maxpool
    "20pct":  "layer1",   # after layer1 (3 Bottleneck blocks)  -> 256-dim
    "40pct":  "layer2",   # after layer2 (4 Bottleneck blocks)  -> 512-dim
    "60pct":  "layer3",   # after layer3 (6 Bottleneck blocks)  -> 1024-dim
    "80pct":  "layer4",   # after layer4 (3 Bottleneck blocks)  -> 2048-dim
    "100pct": "final",    # after avgpool + flatten              -> 2048-dim
}


# ==============================================================================
# 2. LOAD MODEL — plain torchvision resnet50, weights loaded directly (no
...
def load_imagenet_resnet50():
    ...
    backbone.fc = nn.Identity()  # drop the 1000-way ImageNet head -> 2048-dim raw features
    backbone.eval().to(DEVICE)

    embedding_dim = checkpoint.get("embedding_dim", 2048) if isinstance(checkpoint, dict) else 2048
    print(f"Loaded ResNet50 from {CHECKPOINT_PATH} (weights_tag="
...
def get_final_features(backbone, images_tensor, batch_size=32):
    ...
    with torch.no_grad():
        for i in range(0, len(images_tensor), batch_size):
            ...
            f = backbone(batch)  # fc is Identity -> raw 2048-dim pooled features
            all_feats.append(f.cpu().numpy())
    return np.concatenate(all_feats, axis=0)


@torch.no_grad()
def extract_at_layer(backbone, images_tensor, layer_id, batch_size=32):
    backbone.eval()
    all_feats = []
    for i in range(0, len(images_tensor), batch_size):
        batch = images_tensor[i:i + batch_size].to(DEVICE)
        ...
        # final: standard avgpool + flatten (backbone.fc is Identity)
        f = backbone.avgpool(x).flatten(1)
        all_feats.append(f.cpu().numpy())
...
def run_layerwise_rsa(backbone, images_sorted, human_probs_sorted, iterations=(1, 2, 3)):
    ...
    for i in iterations:
        ...
        for l_name, l_id in LAYER_STEPS.items():
            feats = extract_at_layer(backbone, images_sorted, l_id)
...
if __name__ == "__main__":
    ...
    backbone, embedding_dim = load_imagenet_resnet50()
